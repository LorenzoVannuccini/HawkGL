 ...
 (function (name, context, definition) {
    ...
    function HDRImage() {
      ...
      res.__defineSetter__('src',function(val){
        ...
        if (val instanceof ArrayBuffer) loadHDRfromArrayBuffer(val,function(img,width,height){
            HDRdata = img;
        ...
        else if (val.match(/\.hdr$/i)) loadHDR(val,function(img,width,height){
          HDRdata = img;
          this.width  = this.style.width  = width;
          this.height = this.style.height = height;
          context = this.getContext('2d');
    ...
    function m(a,b) { for (var i in b) a[i]=b[i]; return a; };
     ...
     /** Parse a Radiance .HDR file from an array buffer. It completes with a 32bit RGBE buffer.
      * @param {arrayBuffer} arrayBuffer of the .HDR file to load.
      * @param {function} completion completion callback.
      * @returns {XMLHttpRequest} the XMLHttpRequest used to download the file.
      */
      function loadHDRfromArrayBuffer(arrayBuffer, completion)
      {
        var header='',pos=0,d8=new Uint8Array(arrayBuffer),format;
        // read header.  
    ...
    function loadHDR( url, completion) {
      ...
      req.onload  = function() {
        ...
        loadHDRfromArrayBuffer(this.response, completion);
      }
