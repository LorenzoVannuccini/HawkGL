...
glPrimitive.prototype.__buildVertexBuffers = function()
{
    ...
    for(let i = 0, e = this.__vertices.length; i != e; ++i)
    {
...
glPrimitive.prototype.release = function()
{
    if(this.__vertices.length > 0.0)
    {
        if(this.__shouldUpdate) this.__pushToGPU();

        this.__vertexBuffer.release();
        this.__indexBuffer.release();

        this.__vertices.length = 0;
    }
}
...
glPrimitive.prototype.size = function() {
    return this.__nVertices;
}

glPrimitive.prototype.empty = function() {
    return (this.__nVertices == 0);
}
...
glPrimitive.prototype.add = function(vertexData)
{
    ...
    if(nVertices > 0)
    {
        let baseIndex = this.__vertices.length;
        
        this.__vertices.length += nVertices;
        this.__nVertices = (baseIndex + nVertices);
